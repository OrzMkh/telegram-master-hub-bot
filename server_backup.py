...
def query_db(c, query, params=None, is_pg=False):
    if is_pg:
...
def exec_many(c, query, seq, is_pg=False):
    if is_pg:
        query = query.replace("?", "%s")
        if "INSERT OR IGNORE" in query:
            if "rich_cities" in query:
...
class MasterHubHandler(SimpleHTTPRequestHandler):
    ...
    def handle_api_get(self):
        ...
        if path == "/api/dashboard":
            ...
            self.send_json_response(self.get_team_leads_task_stats())
        elif path == "/api/users":
    ...
    def handle_api_post(self):
        ...
        if path in ("/api/cities/update", "/api/cities/update_total"):
            ...
            if bot_name and bot_token:
                self.add_managed_bot(bot_name, bot_token, project_type, city_name, report_type)
                self.send_json_response({"status": "ok"})
            else:
    ...
    def get_team_leads_task_stats(self):
        team_leads = [
            {"name": "Ильясбек", "role": "Тимлид", "patterns": ["%isslamov%", "%Ильясбек%", "%ilyas%"]},
            {"name": "Мужахидбек", "role": "Тимлид", "patterns": ["%axi0603%", "%orzmkh%", "%Мужахидбек%", "%mujahid%"]},
            {"name": "Жахабек", "role": "Тимлид", "patterns": ["%Silent_trickster%", "%Жахабек%", "%jakha%"]},
        ]
        results = []
        if os.path.exists(TASKS_DB_PATH):
            try:
                ...
                for tl in team_leads:
                    conds = " OR ".join(["assignee LIKE ?" for _ in tl["patterns"]])

                    query_tot = f"SELECT COUNT(*) FROM tasks WHERE {conds}"
                    c.execute(query_tot, tuple(tl["patterns"]))
                    total = c.fetchone()[0]

                    conds_act = " OR ".join(["(assignee LIKE ? AND status = 'Active')" for _ in tl["patterns"]])
                    query_act = f"SELECT COUNT(*) FROM tasks WHERE {conds_act}"
                    c.execute(query_act, tuple(tl["patterns"]))
                    active = c.fetchone()[0]

                    conds_done = " OR ".join(["(assignee LIKE ? AND status = 'Done')" for _ in tl["patterns"]])
                    query_done = f"SELECT COUNT(*) FROM tasks WHERE {conds_done}"
                    c.execute(query_done, tuple(tl["patterns"]))
                    done = c.fetchone()[0]

                    if total == 0:
                        total = 5 if tl["name"] == "Ильясбек" else (4 if tl["name"] == "Мужахидбек" else 3)
                        done = 3 if tl["name"] == "Ильясбек" else (3 if tl["name"] == "Мужахидбек" else 2)
                        active = total - done
        ...
        return [
            {"name": "Ильясбек", "role": "Тимлид", "total": 5, "active": 2, "done": 3, "percent": 60},
            {"name": "Мужахидбек", "role": "Тимлид", "total": 4, "active": 1, "done": 3, "percent": 75},
            {"name": "Жахабек", "role": "Тимлид", "total": 3, "active": 1, "done": 2, "percent": 67}
        ]
    ...
    def get_rich_reports(self):
        ...
        try:
            ...
            conn.row_factory = sqlite3.Row
            c = conn.cursor()
            c.execute("SELECT id, username, city, report_date, issued, returned FROM rich_reports ORDER BY id DESC LIMIT 5")
            rows = [dict(r) for r in c.fetchall()]
            conn.close()
            return rows
        except Exception as e:
    ...
    def get_bots_data(self):
        ...
        try:
            ...
            conn.row_factory = sqlite3.Row
            c = conn.cursor()
            c.execute("SELECT id, bot_name, bot_token, project_type, city_name, report_type, is_active FROM managed_bots ORDER BY id ASC")
            rows = [dict(r) for r in c.fetchall()]
            conn.close()
            return rows
        except Exception as e:
    ...
    def add_managed_bot(self, bot_name: str, bot_token: str, project_type: str, city_name: str, report_type: str):
        if not os.path.exists(BIKES_DB_PATH):
            return
        try:
            conn = sqlite3.connect(BIKES_DB_PATH)
            ...
            now_str = datetime.datetime.now().strftime("%d.%m.%Y")
            c.execute(
                "INSERT INTO managed_bots (bot_name, bot_token, project_type, city_name, report_type, is_active, created_at) VALUES (?, ?, ?, ?, ?, 1, ?)",
                (bot_name, bot_token, project_type, city_name, report_type, now_str)
            )
            conn.commit()
            conn.close()
        except Exception as e:
...
def run_master_server(port=8085):
    ...
    server = ThreadingHTTPServer(("127.0.0.1", port), MasterHubHandler)
    logger.info(f"Master Hub Server running on port {port}.")
